...
WORKSPACE =  "${TOPDIR}/../sources/"
FILESPATH =+ "${WORKSPACE}:"

SRC_URI  := "file://le-camera-server"
SRC_URI  += "file://cam-server-env"

S = "${UNPACKDIR}/le-camera-server"
...
EXTRA_OECMAKE += "-DCMAKE_SYSROOT_NATIVE=${UNPACKDIR}/recipe-sysroot-native/"
EXTRA_OECMAKE:append:qcm6490 = "\
     -DTARGET_BOARD_PLATFORM=qcm6490 \
     -DCMAKE_EXE_LINKER_FLAGS='-L${STAGING_LIBDIR}/qcm6490' \
     -DCMAKE_SHARED_LINKER_FLAGS='-L${STAGING_LIBDIR}/qcm6490 -Wl,--hash-style=gnu'\
 "
EXTRA_OECMAKE:append:qcs9100 = "\
     -DTARGET_BOARD_PLATFORM=qcs9100 \
     -DCMAKE_EXE_LINKER_FLAGS='-L${STAGING_LIBDIR}/qcs9100' \
     -DCMAKE_SHARED_LINKER_FLAGS='-L${STAGING_LIBDIR}/qcs9100 -Wl,--hash-style=gnu'\
 "
...
do_install:append () {
    ...
    install ${UNPACKDIR}/cam-server-env -D ${D}/${sysconfdir}/cam-server-env
}
